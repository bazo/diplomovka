...
class ExamStudentListManager {
    ...
    {
        $exam_id = (int)$exam_id;
        $sql = db::select('students.*')->from('students_exams')->join('students')->on('students_exams.student_id = students.id')->where('students_exams.exam_id = %i', $exam_id)->orderBy('students.surname');
        $rows = $sql->fetchAll();
