...
getPanels();}}if(!class_exists('DateTime53',FALSE)){class
DateTime53
extends
DateTime{public
function
...
DibiConnection
extends
DibiObject{private$config;private$driver;private$profiler;private$connected=FALSE;public
function
...
public
function
disconnect(){if($this->connected){$this->driver->disconnect();$this->connected=FALSE;}}final
public
function
...
function
begin($savepoint=NULL){$this->connect();if($this->profiler!==NULL){$ticket=$this->profiler->before($this,IDibiProfiler::BEGIN,$savepoint);}$this->driver->begin($savepoint);if(isset($ticket)){$this->profiler->after($ticket);}}public
function
commit($savepoint=NULL){if($this->profiler!==NULL){$ticket=$this->profiler->before($this,IDibiProfiler::COMMIT,$savepoint);}$this->driver->commit($savepoint);if(isset($ticket)){$this->profiler->after($ticket);}}public
function
rollback($savepoint=NULL){if($this->profiler!==NULL){$ticket=$this->profiler->before($this,IDibiProfiler::ROLLBACK,$savepoint);}$this->driver->rollback($savepoint);if(isset($ticket)){$this->profiler->after($ticket);}}public
function
inTransaction(){$this->connect();return$this->driver->inTransaction();}public
function
...
as$as){if($as!=='[]'&&$as!=='='&&$as!=='->'&&$as!=='|'&&!isset($row[$as])){throw
new
...
as$i=>$as){if($as==='#'){$x=&$x[];}elseif($as==='='){if($x===NULL){$x=(array)$row;$x=&$x[$assoc[$i+1]];$x=NULL;}else{$x=&$x[$assoc[$i+1]];}}elseif($as==='@'){if($x===NULL){$x=clone$row;$x=&$x->{$assoc[$i+1]};$x=NULL;}else{$x=&$x->{$assoc[$i+1]};}}else{$x=&$x[$row->$as];}}if($x===NULL){if($leaf==='='){$x=(array)$row;}else{$x=$row;}}}while($row=$this->fetch());unset($x);return$data;}final
public
function
...
new
DateTime53(is_numeric($value)?date('Y-m-d H:i:s',$value):$value);}elseif(is_numeric($value)){return
date($format,$value);}else{$value=new
DateTime53($value);return$value->format($format);}case
...
function
__wakeup(){$this->setFlags(2);}public
function
...
as$k=>$v){if(is_string($k)){$pair=explode('%',$k,2);$k=$this->delimite($pair[0]).' ';if(!isset($pair[1])){$v=$this->formatValue($v,FALSE);$vx[]=$k.($v==='NULL'?'IS ':'= ').$v;}elseif($pair[1]==='ex'){$vx[]=$k.$this->formatValue($v,'ex');}else{$v=$this->formatValue($v,$pair[1]);$vx[]=$k.($pair[1]==='l'||$pair[1]==='in'?'IN ':($v==='NULL'?'IS ':'= ')).$v;}}else{$vx[]=$this->formatValue($v,'ex');}}return'('.implode(') '.strtoupper($modifier).' (',$vx).')';case'n':foreach($value
...
as$k=>$v){if(is_array($v)){$vx[]=$this->formatValue($v,'ex');}elseif(is_string($k)){$v=(is_string($v)&&strncasecmp($v,'d',1))||$v>0?'ASC':'DESC';$vx[]=$this->delimite($k).' '.$v;}else{$vx[]=$this->delimite($v);}}return
...
DateTime($value);}return$this->driver->escape($value,$modifier);}case'by':case'n':return$this->delimite($value);case'ex':case'sql':$value=(string)$value;$toSkip=strcspn($value,'`[\'":');if(strlen($value)===$toSkip){return$value;}else{return
...
instanceof
ArrayObject){if(isset(self::$modifiers[$clause])){$args=array(self::$modifiers[$clause],$arg);}elseif(is_string(key($arg))){$args=array('%a',$arg);}}}if(array_key_exists($clause,$this->clauses)){$this->cursor=&$this->clauses[$clause];if($args===array(self::REMOVE)){$this->cursor=NULL;return$this;}if(isset(self::$separators[$clause])){$sep=self::$separators[$clause];if($sep===FALSE){$this->cursor=array();}elseif(!empty($this->cursor)){$this->cursor[]=$sep;}}}else{if($args===array(self::REMOVE)){return$this;}$this->cursor[]=$clause;}if($this->cursor===NULL){$this->cursor=array();}array_splice($this->cursor,count($this->cursor),0,$args);return$this;}public
function
clause($clause,$remove=FALSE){$this->cursor=&$this->clauses[self::_formatClause($clause)];if($remove){trigger_error(__METHOD__.'(..., TRUE) is deprecated; use removeClause() instead.',E_USER_NOTICE);$this->cursor=NULL;}elseif($this->cursor===NULL){$this->cursor=array();}return$this;}public
function
removeClause($clause){$this->clauses[self::_formatClause($clause)]=NULL;return$this;}public
function
...
as$clause=>$val){$this->clauses[$clause]=&$val;unset($val);}$this->cursor=&$foo;}}if(!function_exists('array_fill_keys')){function
...
REVISION='5763973 released on 2010-02-16';const
...
new
DateTime53(is_numeric($time)?date('Y-m-d H:i:s',$time):$time);}public
static
function
date($date=NULL){return
new
DateTime53(is_numeric($date)?date('Y-m-d',$date):$date);}public
static
function
...
function
inTransaction(){return(bool)mysql_fetch_field(mysql_query('SELECT @@autocommit',$this->connection));}public
function
...
rollback($savepoint=NULL){$this->query($savepoint?"ROLLBACK TO SAVEPOINT $savepoint":'ROLLBACK');}public
function
inTransaction(){return(bool)mysqli_fetch_field_direct(mysqli_query($this->connection,'SELECT @@autocommit'),0);}public
function
getResource(){return$this->connection;}public
function
escape($value,$type){switch($type){case
...
DibiDriverException(odbc_errormsg($this->connection).' '.odbc_error($this->connection));}odbc_autocommit($this->connection,TRUE);}public
function
inTransaction(){return(bool)odbc_autocommit($this->connection);}public
function
getResource(){return$this->connection;}public
function
escape($value,$type){switch($type){case
...
DibiDriverException("SQLSTATE[$err[0]]: $err[2]",$err[1]);}}public
function
inTransaction(){return(bool)$this->connection->getAttribute(PDO::ATTR_AUTOCOMMIT);}public
function
getResource(){return$this->connection;}public
function
escape($value,$type){switch($type){case
...
inTransaction(){throw
new
NotSupportedException('PostgreSQL driver does not support transaction testing.');}public
function
getResource(){return$this->connection;}public
function
escape($value,$type){switch($type){case
...
DibiDriverException('Reflection requires PostgreSQL 8.');}$this->query("
		...
		");$res=pg_fetch_all($this->resultSet);$this->free();return$res?$res:array();}public
function
...
inTransaction(){throw
new
NotSupportedException('SQLite driver does not support transaction testing.');}public
function
getResource(){return$this->connection;}public
function
escape($value,$type){switch($type){case
...
DibiObject
implements
IDibiDriver{private$connection;private$resultSet;private$fmtDate,$fmtDateTime;private$dbcharset,$charset;public
function
...
function
query($sql){if($this->dbcharset!==NULL){$sql=iconv($this->charset,$this->dbcharset.'//IGNORE',$sql);}$this->resultSet=@$this->connection->query($sql);if($this->connection->lastErrorCode()){throw
new
DibiDriverException($this->connection->lastErrorMsg(),$this->connection->lastErrorCode(),$sql);}return$this->resultSet
instanceof
SQLite3Result?clone$this:NULL;}public
function
...
inTransaction(){throw
new
NotSupportedException('SQLite3 driver does not support transaction testing.');}public
function
getResource(){return$this->connection;}public
function
escape($value,$type){switch($type){case
...
free(){$this->resultSet=NULL;}public
function
getColumnsMeta(){$count=$this->resultSet->numColumns();$res=array();static$types=array(SQLITE3_INTEGER=>'int',SQLITE3_FLOAT=>'float',SQLITE3_TEXT=>'text',SQLITE3_BLOB=>'blob',SQLITE3_NULL=>'null');for($i=0;$i<$count;$i++){$res[]=array('name'=>$this->resultSet->columnName($i),'table'=>NULL,'fullname'=>$this->resultSet->columnName($i),'nativetype'=>$types[$this->resultSet->columnType($i)]);}return$res;}public
function
getResultResource(){return$this->resultSet;}public
function
getTables(){$this->query("
		...
		");$res=array();while($row=$this->fetch(TRUE)){$res[]=$row;}$this->free();return$res;}public
function
getColumns($table){$this->query("
			SELECT sql FROM sqlite_master WHERE type = 'table' AND name = '$table'
			UNION ALL
			SELECT sql FROM sqlite_temp_master WHERE type = 'table' AND name = '$table'");$meta=$this->fetch(TRUE);$this->free();$this->query("PRAGMA table_info([$table])");$res=array();while($row=$this->fetch(TRUE)){$column=$row['name'];$pattern="/(\"$column\"|\[$column\]|$column)\s+[^,]+\s+PRIMARY\s+KEY\s+AUTOINCREMENT/Ui";$type=explode('(',$row['type']);$res[]=array('name'=>$column,'table'=>$table,'fullname'=>"$table.$column",'nativetype'=>strtoupper($type[0]),'size'=>isset($type[1])?(int)$type[1]:NULL,'nullable'=>$row['notnull']=='0','default'=>$row['dflt_value'],'autoincrement'=>(bool)preg_match($pattern,$meta['sql']),'vendor'=>$row);}$this->free();return$res;}public
function
getIndexes($table){$this->query("PRAGMA index_list([$table])");$res=array();while($row=$this->fetch(TRUE)){$res[$row['name']]['name']=$row['name'];$res[$row['name']]['unique']=(bool)$row['unique'];}$this->free();foreach($res
as$index=>$values){$this->query("PRAGMA index_info([$index])");while($row=$this->fetch(TRUE)){$res[$index]['columns'][$row['seqno']]=$row['name'];}}$this->free();$columns=$this->getColumns($table);foreach($res
as$index=>$values){$column=$res[$index]['columns'][0];$primary=FALSE;foreach($columns
as$info){if($column==$info['name']){$primary=$info['vendor']['pk'];break;}}$res[$index]['primary']=(bool)$primary;}return
array_values($res);}public
function
getForeignKeys($table){$this->query("PRAGMA foreign_key_list([$table])");$res=array();while($row=$this->fetch(TRUE)){$res[$row['id']]['name']=$row['id'];$res[$row['id']]['local'][$row['seq']]=$row['from'];$res[$row['id']]['table']=$row['table'];$res[$row['id']]['foreign'][$row['seq']]=$row['to'];$res[$row['id']]['onDelete']=$row['on_delete'];$res[$row['id']]['onUpdate']=$row['on_update'];if($res[$row['id']]['foreign'][0]==NULL){$res[$row['id']]['foreign']=NULL;}}$this->free();return
array_values($res);}public
function
